...
<body class="bg-slate-100 min-h-screen flex items-center justify-center p-6">
  <div class="w-full max-w-md bg-white rounded-2xl shadow p-8">
    ...
    <form action="${pageContext.request.contextPath}/ResetPasswordServlet" method="post" class="space-y-4">
      ...
      <div>
        <%-- Current password required to prevent unauthenticated account takeover.
             A proper email-token reset flow is the long-term replacement. --%>
        <label class="block text-sm font-semibold text-slate-700 mb-1">Current Password</label>
        <input type="password" name="currentPassword" class="w-full border border-slate-300 rounded-lg px-3 py-2" />
      </div>
    ...
    <a href="${pageContext.request.contextPath}/LoginServlet" class="block mt-4 text-sm text-blue-600">Back to Login</a>
  </div>
</body>
</html>
